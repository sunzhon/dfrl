...
--[[
Open the motors of the robot by click this button
--]]
...
function sliderChange(ui,id,newVal)
    ...
    function initParametersSlide()
        ...
        -- controlParameters={ 0.0,0.04,0.04, 0.0,0.04,0.04, 0.0,0.0,0.0,  0.08,0.75,4}
        controlParameters={ 0.0,0.055,0.055, 0.0,0.055,0.055, 0.0,0.0,0.0,  0.08,0.75,4}
    ...
    function sysCall_init()
        ...
        InitialPosition = sim.getObjectHandle("InitialPosition")
        for i = 1, #joint_array do
        ...
        if (not pluginNotFound) then
            local cpgMotorName='motorValues'
    ...
    function sysCall_sensing()
        ...
        for i = 1, #joint_array do
            sensor_array[i]= simGetJointPosition(joint_array[i])
        end	

        for i = 1, #joint_array do
            sensor_array[i+ #joint_array] = simGetJointTargetVelocity(joint_array[i])
        end

        for i = 1, #joint_array do
            sensor_array[i + 2*#joint_array] = simGetJointForce(joint_array[i])
        end	

        for i = 1, #joint_array do
            sensor_array[i + 3*#joint_array] = 12.0; -- get voltage, here assume it as 12.0
        end	

        body_eularAngles=sim.getObjectOrientation(body,-1)
        body_position=sim.getObjectPosition(body,InitialPosition)
        body_velocity=sim.getObjectVelocity(body,InitialPosition)
        for i = 1, #body_eularAngles do
            sensor_array[i + 4*#joint_array ]= body_eularAngles[i]
        end
        for i = 1, #body_position do
            sensor_array[i + 4*#joint_array +#body_eularAngles]= body_position[i]
        end
        for i = 1, #body_velocity do
            sensor_array[i + 4*#joint_array +#body_eularAngles+#body_position]= body_velocity[i]
        end
